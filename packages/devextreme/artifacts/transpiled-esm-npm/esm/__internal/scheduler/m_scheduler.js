...
import { excludeFromRecurrence, getAppointmentTakesAllDay, getPreparedDataItems, isDateAndTimeView, isTimelineView, viewsUtils } from '../scheduler/r1/utils/index';
...
class Scheduler extends Widget {
  ...
  _renderAriaAttributes() {
    ...
    const viewTypeLabel = messageLocalization.format(viewTypeLocalization[this.currentViewType]);
    const label = messageLocalization.format('dxScheduler-ariaLabel',
    // @ts-expect-error
    viewTypeLabel);
    // @ts-expect-error
    this.setAria({
